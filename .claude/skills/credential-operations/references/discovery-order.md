...
Caveat: an MCP tool whose _result_ is the secret value itself (e.g. a
...
_declared_ — never read its value. Use
...
whether the _key_ the task needs is declared, using
...
3. **Check env var _name_ presence only.** Run
...
4. **Stop — do not read `.env` file contents beyond key names.** If step
   ...
   `JIRA_API_TOKEN` is _declared_ (`parse_dotenv_keys.py`) is the next and
