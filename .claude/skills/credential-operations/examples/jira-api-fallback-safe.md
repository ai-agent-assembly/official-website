...
- It would have been _unsafe_ to treat "I have a Jira API token" as
...
- It would have been _dishonest_ to under-report the situation as "done"
  once ordinary issue-level Jira operations (which the same token _does_
  support) went fine, quietly leaving the Goal/Version/Component request
  unfulfilled without saying so.

The safe pattern is: exhaust the ordered fallback for the _specific
operation_, keep every credential channel opaque along the way (Basic
...
_is_ reachable with the available Basic Auth token.
