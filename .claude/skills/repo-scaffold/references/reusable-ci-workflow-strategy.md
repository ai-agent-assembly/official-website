...
- References the corresponding `<stack>-development` skill for the
  _actual_ command sequence and diagnostic ladder, rather than
...
only to _invoke_ them in CI; it does not restate `python-development`'s
...
`profile.stacks`. None of them branch on the _other_ stacks present; a
...
(`references/archetype-composition.md`) only has to decide _whether_ a
