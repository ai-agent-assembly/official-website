...
1. **`docs` archetype gets no coverage config at all.** A documentation-only
   ...
   code whose _test_ coverage would be meaningful. `resolve()` never emits
   ...
   `NOT_APPLICABLE` is proven by the file's _absence_, verified by
...
test_infra_gets_terraform_workflow_only`, which asserts the _complete_
