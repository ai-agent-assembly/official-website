...
2. **Treating navigation/static evidence as correctness proof.**
   ...
   static dependency edges) is evidence for _finding_ code, never proof
   ...
   the graph to decide _where to look and what to run_; let the actual
   gate decide _whether the change works_.
