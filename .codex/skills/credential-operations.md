<!-- horonom:generated -->
<!-- Source: horonomy/.github agents/skills/credential-operations/SKILL.md. Provisional Codex projection shape — see agents/common/README.md. Do not hand-edit — rerun `python3 agents/common/project_skills.py`. -->
...
way in every session, while the _procedure_ for applying it is
...
- As a replacement for a provider-specific integration's own
  documentation — this Skill's provider examples ground the _pattern_,
  they are not a substitute for reading that provider's actual API/CLI
  docs for the operation at hand.
- For secret _values_ that must be provisioned by a human (category B in
