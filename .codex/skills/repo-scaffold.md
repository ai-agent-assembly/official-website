<!-- horonom:generated -->
<!-- Source: horonomy/.github agents/skills/repo-scaffold/SKILL.md. Provisional Codex projection shape — see agents/common/README.md. Do not hand-edit — rerun `python3 agents/common/project_skills.py`. -->
...
- **Archetype** (exactly one, required): the repo's fundamental shape —
  see the eight below. Sets the _default_ capability set and, just as
...
never invents new-repo file _shape_ decisions (archetype/stack/capability
...
- Deciding _whether_ a stack/capability applies to an existing repo based
  on evidence scanning — that's `agents/common/project_skills.py`'s
  `resolve_applicable_skills()`/manifest model for _skill_ applicability
...
3. Apply: `python3 scripts/repo_scaffold.py write <profile.json> <target-dir>`.
   ...
   generator's own provenance marker blocks the _entire_ write — nothing
...
`resolve_applicable_skills()` gates a skill's applicability on _existing_
